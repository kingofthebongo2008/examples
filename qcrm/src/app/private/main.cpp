...
namespace d3d12x
{
    ...
    inline dxgi::ifactory_ptr create_factory()
    {
        using namespace os::windows;
        using namespace d3d12;
    ...
    inline dxgi::ifactory1_ptr create_factory1()
    {
        using namespace os::windows;
        using namespace d3d12;
...
int32_t wWinMain( HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR    lpCmdLine, int       nCmdShow )
{
    ...
    auto factory = create_factory1();
    auto device = create_device(nullptr, D3D_FEATURE_LEVEL_11_0) ;

    descriptor_heap < D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV, D3D12_DESCRIPTOR_HEAP_FLAG_SHADER_VISIBLE > heap1 ( device , 20 );
    descriptor_heap < D3D12_DESCRIPTOR_HEAP_TYPE_SAMPLER> heap2 ( device, 1 );
    descriptor_heap < D3D12_DESCRIPTOR_HEAP_TYPE_RTV> heap3 (device, 1);
    descriptor_heap < D3D12_DESCRIPTOR_HEAP_TYPE_DSV> heap4(device, 1);
