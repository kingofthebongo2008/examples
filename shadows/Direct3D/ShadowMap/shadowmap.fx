...
float	 g_Light_Space_Far;
float	 g_Light_Space_Near;
...
sampler_state
{
    ...
    AddressU = Clamp;
    AddressV = Clamp;
	BorderColor = float4(0.0f, 1.0f, 1.0f, 1.0f);
};
...
float linear_z( float pz, float z, float n, float f, float w )
{
	return pz;
	//return  (  ( z - n  ) / ( f - n ) ) * w;
}
                ...
                out float4 vPosLight : TEXCOORD3 )
{
	...
	float vz = vPosLight.z;
	vPosLight = mul( vPosLight, g_mShadowProj );

	float n = g_Light_Space_Near;
	float f = g_Light_Space_Far;


	vPosLight.z = linear_z ( vPosLight.z, vz, n, f, vPosLight.w );
}
                 ...
                 out float2 Depth : TEXCOORD0 )
{
	...
	oPos.z = linear_z ( oPos.z, vz, n, f, oPos.w );
