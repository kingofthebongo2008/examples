                 ...
                 float4 vPosLight : TEXCOORD3 ) : COLOR
{
    ...
    if( dot( vLight, g_vLightDir ) > g_fCosTheta ) // Light must face the pixel (within Theta)
    {
        ...
        float shadow_sample = tex2D( g_samShadow, ShadowTexC ).r;
        float shadow_sample2 = tex2D( g_samShadow, ShadowTexC +  float2(1.0/SMAP_SIZE, 0) ).r;
        float shadow_sample3 = tex2D( g_samShadow, ShadowTexC  + float2(0, 1.0/SMAP_SIZE)).r;
        float shadow_sample4 = tex2D( g_samShadow, ShadowTexC  + float2(1.0/SMAP_SIZE, 1.0/SMAP_SIZE )).r;

        float LightAmount0 = saturate ( exp ( -80.0 * vPosLight.z / vPosLight.w ) * shadow_sample );
        float LightAmount1 = saturate ( exp ( -80.0 * vPosLight.z / vPosLight.w ) * shadow_sample2 );
        float LightAmount2 = saturate ( exp ( -80.0 * vPosLight.z / vPosLight.w ) * shadow_sample3 );
        float LightAmount3 = saturate ( exp ( -80.0 * vPosLight.z / vPosLight.w ) * shadow_sample4 );
        ...
        */
        ...
        //float LightAttenuation = 1.0f;//abs (dot ( vLight, normalize( vNormal ) ) ) < 5 ? 1 : 0;

        //float LightAmount = LightAttenuation * saturate ( exp ( -80.0 * vPosLight.z / vPosLight.w ) * shadow_sample );//shadow_sample;

        //Filtering 
        float LightAmount = (LightAmount0 + LightAmount1 + LightAmount2 + LightAmount3) / 4;
                ...
                out float4 Color : COLOR )
{
    ...
    // Depth is z / w
    //
    Color = exp ( 80 * (Depth.x / Depth.y) ) + 0.1001f;
}
