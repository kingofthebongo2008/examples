#ifndef __jxr_overlapfilter_h__
#define __jxr_overlapfilter_h__
...
namespace jpegxr
{
    namespace cuda
    {
        __global__ void overlap_filter_4x4( const transforms::pixel* in, transforms::pixel* out, const uint32_t image_pitch, const uint32_t width, const uint32_t height  )
        {
            ...
            transforms::synthesis::overlapfilter4x4
                (
                    &a, &b, &c, &d, 
                    &e, &f, &g, &h, 
                    &i, &j, &k, &l,
        ...
        __global__ void overlap_filter_4_vertical( const transforms::pixel* in, transforms::pixel* out, const uint32_t pitch, const uint32_t width, const uint32_t height)
        {
            auto x = blockIdx.x * blockDim.x + threadIdx.x;
            auto y = blockIdx.y * blockDim.y + threadIdx.y;
        ...
        __global__ void overlap_filter_4_horizontal( const transforms::pixel* in, transforms::pixel* out, const uint32_t pitch, const uint32_t width, const uint32_t height)
        {
            auto x = blockIdx.x * blockDim.x + threadIdx.x;
            auto y = blockIdx.y * blockDim.y + threadIdx.y;
        ...
        __global__ void overlap_filter_2x2_edge( const transforms::pixel* in, transforms::pixel* out, const uint32_t pitch, const uint32_t width, const uint32_t height)
        {
            auto x = blockIdx.x * blockDim.x + threadIdx.x;
            auto y = blockIdx.y * blockDim.y + threadIdx.y;
    ...
    inline void overlapfilter4x4 (  std::shared_ptr< ::cuda::memory_buffer > in, uint32_t width, uint32_t height, uint32_t pitch ) 
    {
        ...
        {
            jpegxr::cuda::overlap_filter_4x4<<<blocks, threads_per_block>>>( in, out, image_pitch, width, height );
        });
    }

    inline void overlapfilter4_vertical(  std::shared_ptr< ::cuda::memory_buffer > in, uint32_t width, uint32_t height, uint32_t pitch ) 
    {
        filter_image( *in, *in, pitch, width, height, [=] 
        ( dim3 blocks, dim3 threads_per_block, const transforms::pixel* in, transforms::pixel* out, const uint32_t image_pitch, const uint32_t width, const uint32_t height )
        {
            jpegxr::cuda::overlap_filter_4_vertical<<<blocks, threads_per_block>>>( in, out, image_pitch, width, height );
        });
        }

    inline void overlapfilter4_horizontal(  std::shared_ptr< ::cuda::memory_buffer > in, uint32_t width, uint32_t height, uint32_t pitch ) 
    {
        filter_image( *in, *in, pitch, width, height, [=] 
        ( dim3 blocks, dim3 threads_per_block, const transforms::pixel* in, transforms::pixel* out, const uint32_t image_pitch, const uint32_t width, const uint32_t height )
        {
            jpegxr::cuda::overlap_filter_4_horizontal<<<blocks, threads_per_block>>>( in, out, image_pitch, width, height );
        });
    }

    inline void overlapfilter2x2_edge (  std::shared_ptr< ::cuda::memory_buffer > in, uint32_t width, uint32_t height, uint32_t pitch ) 
    {
        filter_image( *in, *in, pitch, width, height, [=] 
        ( dim3 blocks, dim3 threads_per_block, const transforms::pixel* in, transforms::pixel* out, const uint32_t image_pitch, const uint32_t width, const uint32_t height )
        {
            jpegxr::cuda::overlap_filter_2x2_edge<<<blocks, threads_per_block>>>( in, out, image_pitch, width, height );
        });
    }
}
