...
namespace QuickFont
{
    ...
    class Builder
    {
        ...
        private QFontShadowConfiguration shadowConfig;
        private Font font;
        ...
        private static void RetargetGlyphRectangleOutwards(BitmapData bitmapData, QFontGlyph glyph, bool setYOffset)
        {
            int startX,endX;
            int startY,endY;
                ...
                for (int i = 0; i < sourceGlyphs.Length; i++)
                {
                    ...
                    if (xPos + rect.Width + 2 * destMargin <= destSheetWidth && yPos + rect.Height + 2 * destMargin <= destSheetHeight)
                    {
                        if (!pre)
                        {
                            ...
                            destGlyphs[i] = new QFontGlyph(pages.Count - 1, new Rectangle(xPos + destMargin, yPos + destMargin, rect.Width, rect.Height), sourceGlyphs[i].yOffset);
                        }
                        else
                        {
        ...
        public QFontData BuildFontData(string saveName)
        {
            ...
            var initialBmp = CreateInitialBitmap(font, maxSize, margin, out initialGlyphs);
            ...
            foreach (var glyph in initialGlyphs){
                RetargetGlyphRectangleInwards(initialBitmapData, glyph, true);
            ...
            if (config.SuperSampleLevels != 1)
            {
                ...
                RetargetAllGlyphs(bitmapPages, glyphs);
            }
            ...
            fontData.KerningPairs = KerningCalculator.CalculateKerning(charSet.ToArray(), glyphs, bitmapPages);
            ...
            if (shadowConfig != null)
                fontData.dropShadow = BuildDropShadow(bitmapPages, glyphs, shadowConfig, charSet.ToArray());
        ...
        private static QFont BuildDropShadow(List<QBitmap> sourceFontSheets, QFontGlyph[] sourceFontGlyphs, QFontShadowConfiguration shadowConfig, char[] charSet)
        {
            ...
            RetargetAllGlyphs(bitmapSheets, newGlyphs);
        ...
        private static void RetargetAllGlyphs(List<QBitmap> pages, QFontGlyph[] glyphs)
        {
            foreach (var glyph in glyphs)
                RetargetGlyphRectangleOutwards(pages[glyph.page].bitmapData, glyph, false);
        }
