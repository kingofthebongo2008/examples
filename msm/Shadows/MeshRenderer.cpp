...
void MeshRenderer::LoadShaders()
{
    ...
    const uint32 NumShaders = (2 * 2 * 2 * 2 * uint32(FixedFilterSize::NumValues) * uint32(ShadowMode::NumValues))
                            + (uint32(AppSettings::NumFilterableShadowModes) * uint32(ShadowMSAA::NumValues))
                    ...
                    for(uint32 filterSize = 0; filterSize < uint32(FixedFilterSize::NumValues); ++filterSize)
                    {
                        for(uint32 shadowMode = 0; shadowMode < uint32(ShadowMode::NumValues); ++shadowMode)
                        {
                            ...
                            if((filterSize > 0 && (shadowMode != uint32(ShadowMode::FixedSizePCF) && shadowMode != uint32(ShadowMode::OptimizedPCF)))
                                ...
                                || (usePlaneBias && AppSettings::UseFilterableShadows(shadowMode)))
                            {
                                continue;
                            }

                            PixelShaderPtr& shader = meshPS[visualizeCascades][usePlaneBias][filterCascades]
                                                           [randomizeOffsets][filterSize][shadowMode];
